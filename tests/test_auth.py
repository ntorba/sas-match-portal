...
def test_registered_user_login(client, session):
    ...
    role = "group_leader"
    email = "test_gl@gl.com"
    gl = add_user(role, email, password)

    scientist_form = LoginForm(**{"email": scientist.email, "password": password})
    ...
    teacher_form = LoginForm(**{"email": gl.email, "password": password})

    response = client.post("/login", data=teacher_form.data)
    assert (
        response.status_code == 302
    ), f"Expected redirect status_code 302, but got {response.status_code} instead."
    assert (
...
def test_login_bad_password(client, session):
    ...
    response = client.post("/login", data=scientist_form.data)
    assert re.search(  # TODO: Do a more robust check by giving flashed messages an idea in html, make beautiful soup object and find those messages specifically
        "This password does not match email.", response.get_data(as_text=True)
...
def test_register_scientist_mismatched_emails(client, session):
    # test that viewing the page renders without template errors
    assert client.get("/register").status_code == 200
    ...
    # make sure there is no message about passwords mismatching
    assert not re.search(  # TODO: Do a more robust check by giving flashed messages an idea in html, make beautiful soup object and find those messages specifically
        "Passwords must match.", response.get_data(as_text=True)
    ), "found unexpected error for mismatched passwords"
